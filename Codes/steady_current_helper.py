...
for folder in folders_list:
    ...
    for file in files_list:
        ...
        for i in current_data:
            d[round(i, 5)] = d.get(round(i, 5), []) + [i] # 将相近的所有值添加到列表中，并建立索引
        Temple_list = list(d.items())
        Temple_list.sort(key=lambda x:len(x[1]), reverse=True)
        model_current = Temple_list[0][0]

        # 4.2 计算最终稳态电流值
        correct_current_data = round((sum(Temple_list[0][1]) / len(Temple_list[0][1]) * 1000 / eval(device_area)), 6)
